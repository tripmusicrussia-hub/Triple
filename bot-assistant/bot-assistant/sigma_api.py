...
У разных поставщиков структура отличается — адаптируйся к тому что видишь.

ЗАДАЧА:
1. Найди таблицу с товарами. Пропускай строки-услуги (доставка, погрузка, экспедирование и т.п.) — только физические товары.
2. qty — из колонки "Количество" / "Кол-во" / "Отгружено". Небольшое число (1–200), может быть дробным (кг). Не путай с ценой или суммой.
3. price — (итоговая сумма строки) ÷ qty, округли до 2 знаков. Используй колонку "Стоимость с НДС" или "Сумма с НДС". Если НДС нет — "Стоимость" или "Сумма". Пример: сумма строки 1975, qty 5 → price = 395.00.
4. qty_str — исходное значение количества с единицей измерения из накладной, например "5 шт" или "1.5 кг".
5. ВАЖНО: включай в JSON ТОЛЬКО строки где qty явно указано и больше нуля. Строки с пустой ячейкой количества — это товары не из этой поставки, их НЕ включай вообще.
6. Строки "Итого", "Всего", "НДС итого" — не включай в товары.
7. name — ДОСЛОВНО из накладной, включая граммовку и фасовку.
8. supplier — тот кто ПРОДАЁТ (поле "Продавец" или "Поставщик" в шапке), не покупатель.
9. total_sum — итоговая сумма всего документа.
