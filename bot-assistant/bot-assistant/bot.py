...
def _load_bundle_carts() -> None:
    """Load saved carts from disk. Если файл старше BUNDLE_CART_TTL_DAYS —
    игнорируем content (юзеры скорее всего забыли о корзинах за 2 недели,
    бессмысленно показывать stale state). Это file-level TTL — простое
    решение без schema-changes на per-user timestamp.
    """
    if not os.path.exists(BUNDLE_CART_PATH):
        return
    try:
        # Heuristic TTL по mtime файла: если последний save был >14 дней назад,
        # значит сервис простаивал (или Render redeploy reset все carts) →
        # старт чистый. Защита от загрузки месячных stale carts.
        mtime = os.path.getmtime(BUNDLE_CART_PATH)
        age_days = (time.time() - mtime) / 86400
        if age_days > BUNDLE_CART_TTL_DAYS:
            logger.info(
                "bundle_cart: file age %.1f days > TTL %d, starting clean",
                age_days, BUNDLE_CART_TTL_DAYS,
            )
            return
...
def _save_bundle_carts() -> None:
    ...
    Atomic: tmp-файл + os.replace — защита от corruption если процесс
    умрёт mid-write (Render OOM kill, etc).
    """
    tmp_path = BUNDLE_CART_PATH + ".tmp"
    try:
        with open(tmp_path, "w", encoding="utf-8") as f:
            ...
            try:
                ...
                pass  # не все FS поддерживают fsync (overlayfs)
        os.replace(tmp_path, BUNDLE_CART_PATH)  # atomic на Linux+Windows
    except Exception:
...
# TTL для bundle_cart: чистим entries старше N дней. Без cleanup dict растёт
# безгранично (юзеры добавляют биты но не покупают → старые carts остаются
# в RAM forever). Schema: {user_id_str: list[beat_id]} — нет timestamp в
# структуре, поэтому используем mtime файла + heuristic: при load проверяем
# последний modify через os.stat и если файл старше TTL — стираем dict.
# Для per-user TTL надо менять schema на {uid: {ids, ts}} — отложено.
BUNDLE_CART_TTL_DAYS = 14  # Юзер собирает корзину за 1-3 сессии, 14 дней щедро.
...
def kb_after_beat(beat_id, content_type="beat", user_id: int | None = None):
    ...
    if content_type == "beat":
        ...
        # i18n: button labels автоматически по языку юзера.
        if user_id is not None:
            cart = _cart_get(user_id)
            import i18n as _i
            _lang = _i.get_user_lang(user_id)
            _price = licensing.PRICE_BUNDLE3_RUB
            if len(cart) >= BUNDLE_TOTAL:
                rows.append([InlineKeyboardButton(
                    _i.t("cart_btn_ready_buy", _lang, price=_price),
                    callback_data="cart_buy",
                )])
                rows.append([InlineKeyboardButton(
                    _i.t("cart_btn_change", _lang, n=len(cart)),
                    callback_data="cart_show",
                )])
            elif beat_id in cart:
                rows.append([InlineKeyboardButton(
                    _i.t("cart_btn_in", _lang, n=len(cart), total=BUNDLE_TOTAL),
                    callback_data="cart_show",
                )])
            else:
                rows.append([InlineKeyboardButton(
                    _i.t("cart_btn_add", _lang, price=_price, n=len(cart), total=BUNDLE_TOTAL),
                    callback_data=f"cart_add_{beat_id}",
                )])
        rows.append([
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("cart_add_"):
        ...
        _price = licensing.PRICE_BUNDLE3_RUB
        full_single = BUNDLE_TOTAL * licensing.PRICE_MP3_RUB
        saving = full_single - _price
        if len(cart) >= BUNDLE_TOTAL:
            await query.answer(
                _i.t("cart_alert_ready", _lang, n=len(cart), total=BUNDLE_TOTAL),
                show_alert=False,
            )
            chosen_beats = [beats_db.get_beat_by_id(bid) for bid in cart[:BUNDLE_TOTAL]]
            ...
            if len(chosen_beats) == BUNDLE_TOTAL:
                ...
                try:
                    await bot.send_message(
                        user_id,
                        (
                            f"{_i.t('bundle_ready_title', _lang, price=_price, saving=saving, full=full_single)}\n\n"
                            f"<b>3 beats / 3 бита:</b>\n{names_block}\n\n"
                            f"{_i.t('bundle_ready_pick_pay', _lang)}"
                        ),
                        parse_mode="HTML",
            ...
            need = BUNDLE_TOTAL - len(cart)
            await query.answer(
                _i.t("cart_alert_added", _lang,
                     n=len(cart), total=BUNDLE_TOTAL, need=need,
                     price=_price, saving=saving),
                show_alert=False,
            )
        # Обновляем карточку — кнопка должна стать «✅ В наборе» / «✅ Купить набор»
        try:
...
async def cmd_yt_refresh_old(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Регенерим description через replace legacy цен на актуальные из
        # licensing.PRICE_*. Не LLM-переписываем — сохраняем оригинальный flow.
        # licensing.PRICE_* константы — single source of truth, при изменении
        # цен в одном месте refresh подхватит автоматически (не надо менять
        # hardcoded строки в этом handler'е).
        import beat_post_builder as _bpb
        cur_stars = licensing.PRICE_MP3_STARS
        cur_usdt_g = f"{licensing.PRICE_MP3_USDT:g}"
        cur_rub = licensing.PRICE_MP3_RUB
        new_full = f"{cur_stars}⭐ / {cur_usdt_g} USDT / {cur_rub}₽"
        new_short = f"({cur_stars}⭐ / {cur_usdt_g} USDT)"
        new_desc = old_desc
        # Старые legacy formats (Этап A: 500⭐ / 7 USDT)
        new_desc = new_desc.replace("500⭐ / 7 USDT", new_full)
        new_desc = new_desc.replace("(500 / 7 USDT)", new_short)
        new_desc = new_desc.replace("500 / 7 USDT", new_full)
        # Промежуточные varianты до текущих цен (Этап B: 1500⭐ / 20 USDT
        # без RUB) — добавим RUB чтобы все 3 currency показывались.
        old_b = f"{cur_stars}⭐ / {cur_usdt_g} USDT"  # без / RUB
        if old_b in new_desc and new_full not in new_desc:
            new_desc = new_desc.replace(old_b, new_full)
